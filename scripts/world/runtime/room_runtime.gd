...
const SPAWN_POINT_SCRIPT: Script = preload("res://scripts/world/entities/spawn_point.gd")
const ROOM_GRID: Script = preload("res://scripts/world/data/room_grid.gd")
...
func setup_room(
	...
	_entity_state_sink = entity_state_source
	if not _is_room_data(room_data) or room_data.scene_path.is_empty() or origin_chunk == null:
		return false
	...
	_world_id = world_id
	_origin_chunk = Vector2i(origin_chunk)
	name = room_data.room_id
	position = Vector2(_origin_chunk * ROOM_GRID.DEFAULT_CHUNK_SIZE_PIXELS)
...
func get_room_cell_rect() -> Rect2i:
	if _room_data == null:
		return Rect2i()
	return ROOM_GRID.get_room_cell_rect(_origin_chunk, _room_data.room_size_chunks)
...
func persist_entity_states() -> void:
	var root := get_entities_root()
	if root == null:
		return
	for child: Node in root.get_children():
		_persist_entity_tree(child)
...
func _setup_entity_tree(entity: Node) -> void:
	if entity.has_method("setup_entity"):
		entity.call("setup_entity", {
			"world_id": _world_id,
			"room_id": get_room_id(),
			"entity_state_sink": _get_entity_state_sink(),
		})
		if "persistent" in entity and bool(entity.get("persistent")) and entity.has_method("get_save_key"):
...
func _persist_entity_tree(entity: Node) -> void:
	if "persistent" in entity and bool(entity.get("persistent")) and entity.has_method("commit_save_state"):
		entity.call("commit_save_state")
	for child: Node in entity.get_children():
		_persist_entity_tree(child)
...
func _get_entity_state_sink() -> Object:
	if is_instance_valid(_entity_state_sink):
		return _entity_state_sink
	return _get_entity_state_source()
