...
func start(world: WorldData = null, snapshot: RefCounted = null) -> bool:
	...
	if fog != null and fog.has_method("bind_player"):
		fog.call("bind_player", player)
	if fog != null and fog.has_method("bind_persistence_source"):
		fog.call("bind_persistence_source", self)
	if fog != null and fog.has_method("bind_settings_source"):
		fog.call("bind_settings_source", _settings_source)
	...
	var callback := Callable(self, "_on_current_room_changed").bind(terrain_runtime, fog, camera)
...
func bind_settings_source(source: Object) -> void:
	...
	var fog := get_node_or_null(fog_path)
	if fog != null and fog.has_method("bind_settings_source"):
		fog.call("bind_settings_source", source)
...
func _on_current_room_changed(room_id: String, terrain_runtime: Node, fog: Node, camera: Node) -> void:
	if _active_world != null:
		if _bind_fog_room(_active_world, room_id, terrain_runtime, fog):
...
func _bind_save_flush(runtime: Node) -> void:
	...
	if manager == null or not manager.has_signal("snapshot_committing"):
		return
	var callback := Callable(runtime, "persist_loaded_entity_states")
